...
public class RewriteFilter extends AbstractFilter
{
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException
    {
        ...
        {
            response = new RewritingResponse((HttpServletRequest) request, (HttpServletResponse) response, allPatterns);

            done = process((HttpServletRequest) request, (HttpServletResponse) response, allPatterns);
        }
    ...
    public boolean process(HttpServletRequest request, HttpServletResponse response, List<Pattern> patterns) throws IOException,
            ServletException
    {
        ...
        if (fullPath.indexOf(".") > 0)
        {
            if(patterns != null)
            {
                ...
                {
                    if(pattern.view.equals(fullPath) == false)
                    {
                        return false;
                    }
                }
    ...
    public static void main(String[] args)
    {
        ...
        if (ass.indexOf(".") > 0)
        {
            String vm = "*" + ass.substring(ass.indexOf("."));
