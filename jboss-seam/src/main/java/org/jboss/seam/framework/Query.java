 ...
 * view, or accessed by some intermediate Seam component.
 * 
 * @author Gavin King
 *
 */
public abstract class Query<T, E> extends PersistenceController<T> // TODO:
// extend
// MutableController!
{
    ...
    protected String getCountEjbql()
    {
        ...
        // to be JPA-compliant, we need to make this query like
        ...
        return new StringBuilder(ejbql.length() + 15).append("select count(").append(subject).append(") ")
                .append(ejbql.substring(fromLoc, whereLoc).replace("join fetch", "join")).append(ejbql.substring(whereLoc, groupLoc))
                .toString().trim();
    }
     ...
     * property for component configuration. Use setRestrictions() for the
     * later.
     */
    ...
    private String sanitizeOrderColumn(String columnName)
    {
        ...
        {
            throw new IllegalArgumentException("invalid order column (\"" + columnName + "\" must match the regular expression \""
    ...
    private static boolean isAnyParameterDirty(List<ValueExpression> valueBindings, List<Object> lastParameterValues)
    {
        ...
        for (int i = 0; i < valueBindings.size(); i++)
        {
            ...
            // treat empty strings as null, for consistency with
